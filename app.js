const VAT_RATE = 0.12;
const SAMPLE_FILE = 'Заказы с товарами - ДОП1 (2).xls';
...
const INVOICE_PRODUCTS = [
  { name: 'Бумажное Полотенце 365 kun 2сл 2рул', price: 7200 },
  { name: 'САЛФЕТКА СТОЛ. 24*24 365 kun 100ШТ', price: 2975.9 },
  { name: 'Туалетная бумага 365 kun 2-слой шт', price: 1815.6 },
  { name: 'БУМАЖ. ПОЛОТЕНЦЕ 365kun BIG SIZE 2СЛ', price: 10430 },
  ...
  { name: 'САЛФЕТКИ 365 kun 27X27 100ШТ', price: 3426.07 },
  ...
  { name: 'Сал.дисп 365 kun целлюлозные 20x20 180шт', price: 7544.64 },
];
...
const els = {
  ...
  resultsSearch: document.querySelector('#resultsSearch'),
  resultsSelectionInfo: document.querySelector('#resultsSelectionInfo'),
  selectVisibleResultsBtn: document.querySelector('#selectVisibleResultsBtn'),
  clearSelectionBtn: document.querySelector('#clearSelectionBtn'),
  invoiceSearch: document.querySelector('#invoiceSearch'),
  invoiceSelectionInfo: document.querySelector('#invoiceSelectionInfo'),
  selectVisibleInvoiceBtn: document.querySelector('#selectVisibleInvoiceBtn'),
  clearInvoiceSelectionBtn: document.querySelector('#clearInvoiceSelectionBtn'),
  ...
  exportSelectedInvoiceExcelBtn: document.querySelector('#exportSelectedInvoiceExcelBtn'),
...
els.exportInvoiceExcelBtn.addEventListener('click', () => exportInvoiceTemplateExcel('all'));
els.exportSelectedInvoiceExcelBtn.addEventListener('click', () => exportInvoiceTemplateExcel('selected'));
...
els.invoiceContent.addEventListener('change', (event) => {
  if (!event.target.matches('.invoice-select input[type="checkbox"]')) return;
  const key = event.target.dataset.storeKey;
  if (!key) return;
  if (event.target.checked) state.selectedStoreKeys.add(key);
  else state.selectedStoreKeys.delete(key);
...
els.invoiceSearch.addEventListener('input', renderInvoice);
els.selectVisibleResultsBtn.addEventListener('click', () => selectFilteredStores('results'));
els.selectVisibleInvoiceBtn.addEventListener('click', () => selectFilteredStores('invoice'));
els.clearSelectionBtn.addEventListener('click', clearStoreSelection);
els.clearInvoiceSelectionBtn.addEventListener('click', clearStoreSelection);
...
function parseRows(rows) {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    if (shopHeader) {
      ...
      if (dataRow) {
        ...
        if (storeName) {
          ...
          addOrderNumber(currentStore, findOrderNumberForStore(rows, rowIndex, shopHeader.nameIndex));
        }
      }
      productColumns = null;
      return;
    ...
    if (directShopIndex !== -1 && !currentStore) {
      ...
      if (name && name.toLowerCase() !== 'магазин') {
        currentStore = getStore(storeMap, name, 0);
        addOrderNumber(currentStore, findOrderNumberForStore(rows, rowIndex, directShopIndex + 1));
      }
    }
...
function addOrderNumber(store, value) {
  const orderNumber = cleanOrderNumber(value);
  if (!orderNumber || store.orderNumbers.includes(orderNumber)) return;
  store.orderNumbers.push(orderNumber);
}
...
function findOrderNumberForStore(rows, headerRowIndex, storeColumnIndex) {
  const start = Math.max(0, headerRowIndex - 6);
  for (let rowIndex = headerRowIndex; rowIndex >= start; rowIndex -= 1) {
    const row = rows[rowIndex] || [];
    const sameColumn = cleanOrderNumber(row[storeColumnIndex]);
    if (sameColumn) return sameColumn;

    for (let column = storeColumnIndex; column >= Math.max(0, storeColumnIndex - 3); column -= 1) {
      const nearLeft = cleanOrderNumber(row[column]);
      if (nearLeft) return nearLeft;
    }

    for (let column = storeColumnIndex + 1; column <= Math.min(row.length - 1, storeColumnIndex + 3); column += 1) {
      const nearRight = cleanOrderNumber(row[column]);
...
function filterStoresByQuery(stores, query) {
  ...
  if (!words.length) return stores;
  return stores.filter((store) => {
    const text = normalizeText(store.name);
    return words.every((word) => text.includes(word));
  });
}

function getSelectedStores(stores = rebuildStoresForCurrentGrouping()) {
  return stores.filter((store) => state.selectedStoreKeys.has(storeKey(store)));
}

function getFilteredStoresForView(view) {
  const stores = rebuildStoresForCurrentGrouping();
  const query = view === 'invoice' ? els.invoiceSearch.value : els.resultsSearch.value;
  return filterStoresByQuery(stores, query);
}

function selectFilteredStores(view) {
  const stores = getFilteredStoresForView(view);
  stores.forEach((store) => state.selectedStoreKeys.add(storeKey(store)));
  renderSummary();
  renderResults();
  renderInvoice();
}
...
function syncSelectionControls() {
  const stores = rebuildStoresForCurrentGrouping();
  const selectedStores = getSelectedStores(stores);
  const selectedTotal = totals(selectedStores.flatMap((store) => store.products));
  const baseText = `Tanlangan: ${selectedStores.length}. Jami mahsulot: ${formatQuantity(selectedTotal.quantity)}`;
  if (els.resultsSelectionInfo) {
    const resultsVisible = filterStoresByQuery(stores, els.resultsSearch.value).length;
    els.resultsSelectionInfo.textContent = `Ko‘ringan: ${resultsVisible}. ${baseText}`;
  }
  if (els.invoiceSelectionInfo) {
    const invoiceVisible = filterStoresByQuery(stores, els.invoiceSearch.value).length;
    els.invoiceSelectionInfo.textContent = `Ko‘ringan: ${invoiceVisible}. ${baseText}`;
  }
}
...
function renderSummary() {
  ...
  const selectedStores = getSelectedStores(rebuildStoresForCurrentGrouping());
  const selectedTotal = totals(selectedStores.flatMap((store) => store.products));
  setStatus(`<b>${state.fileName}</b> успешно проанализирован.`);
  els.summaryList.innerHTML = [
    `Магазины: <b>${state.stores.length}</b>`,
    `Tanlangan joylar: <b>${selectedStores.length}</b>`,
    `Tanlangan jami mahsulot: <b>${formatQuantity(selectedTotal.quantity)}</b>`,
...
function renderResults() {
  ...
  const stores = rebuildStoresForCurrentGrouping();
  const availableKeys = new Set(stores.map((store) => storeKey(store)));
  state.selectedStoreKeys.forEach((key) => {
    if (!availableKeys.has(key)) state.selectedStoreKeys.delete(key);
  });

  const filteredStores = filterStoresByQuery(stores, els.resultsSearch.value);
  els.resultsContent.className = 'store-grid';
  els.resultsContent.innerHTML = filteredStores.length
    ? filteredStores.map((store) => storeTable(store)).join('')
    : '<div class="empty-state">Qidiruv bo‘yicha joy topilmadi.</div>';
  syncSelectionControls();
}
...
function formatOrderNumbers(store) {
  return (store.orderNumbers || []).filter(Boolean).join(', ');
}
...
function renderInvoice() {
  ...
  const stores = rebuildStoresForCurrentGrouping();
  const filteredStores = filterStoresByQuery(stores, els.invoiceSearch.value);
  els.invoiceContent.className = 'invoice-preview';
  els.invoiceContent.innerHTML = filteredStores.length
    ? filteredStores
  ...
  return `
    <article class="invoice-document">
      ...
      <div class="invoice-meta">
        ...
        <div><b>Номер заказ:</b> ${escapeHtml(orderNumberText || '-')}</div>
...
async function exportInvoiceTemplateExcel(mode = 'all') {
  if (!state.stores.length) {
    showError('Сначала загрузите и разберите файл заказов.');
    return;
  }
  if (mode === 'selected' && !state.selectedStoreKeys.size) {
    showError('Nakladnoy qilish uchun avval 3-menyuda joylarni belgilang.');
    return;
  }
  if (!window.XLSX) {
    showError('Библиотека XLSX не загружена. Проверьте подключение к интернету.');
  ...
  const buttons = [els.exportExcelBtn, els.exportInvoiceExcelBtn, els.exportSelectedInvoiceExcelBtn];
  buttons.forEach((button) => { button.disabled = true; });

  try {
    setStatus(mode === 'selected'
      ? 'Шаблон загружается, выбранные магазины раскладываются по отдельным листам...'
      : 'Шаблон загружается, магазины раскладываются по отдельным листам...');
    const templateBuffer = await loadInvoiceTemplateBuffer();
    const sourceStores = mode === 'selected'
      ? getSelectedStores(rebuildStoresForCurrentGrouping())
      : rebuildStoresForCurrentGrouping();
    if (!sourceStores.length) {
      showError('Tanlangan joylar topilmadi. Belgilarni tekshiring.');
      return;
    }
    ...
    downloadBlob(
      output,
      `${mode === 'selected' ? 'tanlangan-' : ''}schf-korzinka-${Date.now()}.xlsx`,
      'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    );
    setStatus(`<b>${state.fileName}</b>: ${stores.length} ${mode === 'selected' ? 'tanlangan' : 'jami'} joy nakladnoyga выгружено.`);
  } catch (error) {
...
function fillInvoiceSheetCopy(sheet, offset, store, invoiceNumber) {
  ...
  setSheetCell(sheet, `H${contractRow}`, '      к Договору № 15/365 от «28» ноября 2022г.');
...
function fillInvoiceXmlCopy(documentXml, cellMap, offset, store, invoiceNumber) {
  ...
  setXmlCell(documentXml, cellMap, `H${contractRow}`, '      к Договору № 15/365 от «28» ноября 2022г.');
